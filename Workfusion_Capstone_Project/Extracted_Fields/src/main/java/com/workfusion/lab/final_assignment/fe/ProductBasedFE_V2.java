...
@FeatureName(ProductBasedFE_V2.FEATURE_NAME)
public class ProductBasedFE_V2 implements FeatureExtractor<Token> {

    public final static String FEATURE_NAME = "product";
...
//            if ( col_index2 == 0 && row_index2 != 0 && isStringOnlyAlphabet(col_text)) {
//                for(int ij=0;ij<nextcells.size();++ij){
//                    if(nextcells.get(ij).getColumnIndex()>col_index2){
//                        String temp = nextcells.get(ij).getText().replace("\n", "").replace("$","");
//
...
//}
    ...
    public Collection<Feature> extract(Document document, Token element) {
        ...
        List<Cell> lines = document.findCovering(Cell.class, element);
        if (!lines.isEmpty()) {
            int col_index = lines.get(0).getColumnIndex();
            int row_index = lines.get(0).getRowIndex();
            String col_text = lines.get(0).getText().replace("\n", "").replace(" ", "");
            if (col_index == 0 && row_index != 0 && isStringOnlyAlphabet(col_text)) {
                String temp = col_text;
                features.add(new Feature(FEATURE_NAME, 1));
            }
        }
        return features;
